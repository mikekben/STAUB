...
int main(int argc, char* argv[])
{
  ...
  //Z3_get_version(&major, &minor, &bn, &rn);
  ...
  if(HasFlag(argc, argv, "-h"))
    ...
    if (useInteger)
    {
      ...
      if (useIntegerWidth == -1) //Abstract interpretation case
      {
        ...
        largest.print(rso, true);
        stats.push_back(rso.str());

        aiResult.print(rso, true);
        stats.push_back(rso.str());
    ...
    sol.set(":timeout", 10000u);
    sol.set(":max_memory", 5120u);
