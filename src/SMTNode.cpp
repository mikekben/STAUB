...
namespace STAUB
{
    ...
    inline func_decl SMTNode::Z3_bvnego(int width) { return function("bvnego", scx.bv_sort(width), scx.bool_sort()); }
    inline func_decl SMTNode::Z3_bvsaddo(int width) { return function("bvsaddo", scx.bv_sort(width), scx.bv_sort(width), scx.bool_sort()); }
    inline func_decl SMTNode::Z3_bvssubo(int width) { return function("bvssubo", scx.bv_sort(width), scx.bv_sort(width), scx.bool_sort()); }
    inline func_decl SMTNode::Z3_bvsmulo(int width) { return function("bvsmulo", scx.bv_sort(width), scx.bv_sort(width), scx.bool_sort()); }
    inline func_decl SMTNode::Z3_bvsdivo(int width) { return function("bvsdivo", scx.bv_sort(width), scx.bv_sort(width), scx.bool_sort()); }

    //bvsmulo   bvsdivo
    //bvuaddo bvsaddo
    //function("bvnego", scx.bv_sort(width), scx.bool_sort());
    ...
    expr RealNode::ToSMTFloat(z3::sort type, std::map<std::string, expr> svariables)
    {
        ...
        else if (IsConstant())
        {
            ...
            return expr(scx,Z3_mk_fpa_to_fp_real(scx,scx.fpa_rounding_mode(),scx.real_val(str.c_str()),type)).simplify();
        }
        else //Expression
        {
        ...
        else if (IsConstant())
        {
            ...
            if (isNegative)
            {
                if (!forSlot)
                {
                    ...
                    sol->add(!SMTNode::Z3_bvnego(width)(e));

                    //sol->add(!expr(scx, mk_bv_neg_ovfl(scx, e)));
                    ...
                    sol->add(!expr(scx,mk_bvneg_overflow(scx,e)));*/
                }
